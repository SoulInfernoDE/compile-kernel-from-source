...
cd kernel
read -p "Which kernel version do you want to compile? (example: 5.19-rc1) " KERNEL_VERSION
echo 'kernel version you entered: '$KERNEL_VERSION'_android'
git clone https://kernel.source.codeaurora.cn/pub/scm/linux/kernel/git/torvalds/linux.git --depth 1 --branch v$KERNEL_VERSION
cd linux # we open the kernel source code folder downloaded by git
...
wget -O remove_ashmem.patch https://kernel.source.codeaurora.cn/pub/scm/linux/kernel/git/torvalds/linux.git/patch/?id=721412ed3d819e767cac2b06646bf03aa158aaec
echo ''
